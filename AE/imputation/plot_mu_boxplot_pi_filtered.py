"""
Plot mu values boxplot for essential vs non-essential genes.
Filters for original zero values with pi < 0.5.
Based on imputation_mu.py structure.
"""
import json
import os
import sys
...
def map_reconstruction_position_to_original_window(
    ...
    if position_mode == POSITION_MODE_INDEX:
        ...
        # center mode: metadata/reconstruction already stores center-like genomic positions
...
def get_genes_overlapping_position(
    ...
    """
    Find all genes overlapping with a reconstructed position's original window.
...
def assign_mu_to_genes(
    ...
    genes_by_chr: Dict,
    chr_name: str,
    filter_by_pi_threshold: bool = True,
    ...
    Args:
        ...
        filter_by_pi_threshold: If True, only keep mu values where pi < pi_threshold
        pi_threshold: Pi threshold value (default 0.5)
        filter_mu_by_original_zeros:
            If True, only keep mu values whose mapped original moving-average
    ...
    raw_df = pd.read_csv(
        csv_file,
        usecols=["position", "mu", "pi"],
        low_memory=False,
        dtype={"position": "string", "mu": "string", "pi": "string"}
    )
    df = raw_df.copy()
    ...
    invalid_mask = df["position"].isna() | df["mu"].isna() | df["pi"].isna()
    invalid_count = int(invalid_mask.sum())
    if invalid_count:
        print(f"Warning: Found {invalid_count} invalid rows in {csv_file} (non-numeric position, mu, or pi). These rows will be skipped.")

    df = df.loc[~invalid_mask, ["position", "mu", "pi"]]
    if df.empty:
    ...
    for position, mu_value, pi_value in df.itertuples(index=False, name=None):
        position = int(position)
        mu_value = float(mu_value)
        pi_value = float(pi_value)

        # Filter by pi threshold
        if filter_by_pi_threshold and pi_value >= pi_threshold:
            continue

        # Filter by original zeros
...
def process_single_split(
    ...
    Args:
        ...
        filter_by_pi_threshold: If True, only include mu where pi < pi_threshold
        pi_threshold: Pi threshold value (default 0.5)
    ...
    for strain in strains:
        ...
        for chr_name in CHROMOSOMES:
            ...
            genes_mu = assign_mu_to_genes(
                ...
                genes_by_chr,
                chr_name,
                filter_by_pi_threshold=filter_by_pi_threshold,
...
def create_comparison_boxplot(
    ...
    """
    # Calculate 95th percentile for y-axis limit (removes top 5% from view)
    all_data = np.concatenate([essential_mu, nonessential_mu])
    if len(all_data) > 0:
        y_limit = np.percentile(all_data, 95)
    else:
        y_limit = None
    ...
    bp = ax.boxplot(
        ...
        patch_artist=True,
        widths=0.6,
        showfliers=False  # Don't show outliers beyond whiskers
    )
    ...
    # Set y-axis limit to 95th percentile for better visibility
...
def run_full_analysis(
    reconstruction_root_dir: str = "Data/reconstruction/ZINBAE_layers1600_ep144_noise0.150_muoff0.000",
    output_dir: str = "AE/results/mu_boxplot_pi_filtered",
    filter_by_pi_threshold: bool = True,
    pi_threshold: float = 0.5,
    filter_mu_by_original_zeros: bool = True,
    original_data_root_dir: str = "Data/combined_strains",
    ...
    combined/per-strain visualizations.

    Args:
        reconstruction_root_dir: Root directory containing split folders
            (e.g., train, validation/val, test)
        output_dir: Output directory for figures
        filter_by_pi_threshold: If True, only use mu values where pi < pi_threshold
        pi_threshold: Pi threshold value (default 0.5)
        filter_mu_by_original_zeros:
    ...
    print(f"  - Pi threshold filter: {filter_by_pi_threshold} (threshold={pi_threshold})")
    print(f"  - Original zero filter: {filter_mu_by_original_zeros}")
    if filter_mu_by_original_zeros:
        print(
            f"    Using original data from {original_data_root_dir} "
