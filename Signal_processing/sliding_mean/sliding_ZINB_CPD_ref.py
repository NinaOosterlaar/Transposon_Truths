...
def sliding_ZINB_CPD_ref(data, window_size, overlap, threshold, eps=1e-10, theta_global=None, tol=1e-6, max_iter=10, pi_file="Signal_processing/sample_data/SATAY_synthetic/pi_values.csv"):
    ...
    for start in range(0, n - 2 * window_size + 1, step_size):
        ...
        # init
        pi1 = np.mean(pi_values[start : start + window_size])  # average pi in the window
        pi2 = np.mean(pi_values[start + window_size : start + 2 * window_size])  # average pi in the window
        pi0 = np.mean(pi_values[start : start + 2 * window_size])  # average pi in the combined window
