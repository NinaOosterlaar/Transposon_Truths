...
# Paths
DATA_DIR = Path(__file__).parent / "boundary_alignment"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Generating Boundary Alignment Plots")
    logger.info("=" * 80)
    ...
    if not distance_stats_file.exists():
        logger.error(f"Distance stats file not found: {distance_stats_file}")
        logger.error("Please run changepoint_boundary_alignment_analysis.py first!")
        return
    
    distance_stats_df = pd.read_csv(distance_stats_file)
    logger.info(f"Loaded distance statistics: {len(distance_stats_df)} rows")
    
    if genes_with_cp_file.exists():
        genes_with_cp_df = pd.read_csv(genes_with_cp_file)
        logger.info(f"Loaded genes analysis: {len(genes_with_cp_df)} rows")
    else:
        genes_with_cp_df = None
        logger.warning("Genes with CP file not found, skipping...")
    
    if cps_with_boundary_file.exists():
        cps_with_boundary_df = pd.read_csv(cps_with_boundary_file)
        logger.info(f"Loaded change points analysis: {len(cps_with_boundary_df)} rows")
    else:
        cps_with_boundary_df = None
        logger.warning("CPs with boundary file not found, skipping...")
    
    if boundary_to_cp_file.exists():
        boundary_to_cp_df = pd.read_csv(boundary_to_cp_file)
        logger.info(f"Loaded boundary-to-CP statistics: {len(boundary_to_cp_df)} rows")
    else:
        boundary_to_cp_df = None
        logger.warning("Boundary-to-CP file not found, skipping...")
    ...
    logger.info("1. Distance vs Threshold (mean) - Change Points to Boundaries...")
    ...
    logger.info("2. Distance vs Threshold (median) - Change Points to Boundaries...")
    ...
    if boundary_to_cp_df is not None:
        logger.info("3. Distance vs Threshold (mean) - Boundaries to Change Points...")
    ...
    if boundary_to_cp_df is not None:
        logger.info("4. Distance vs Threshold (median) - Boundaries to Change Points...")
    ...
    if genes_with_cp_df is not None:
        logger.info("5. Genes with Nearby Change Points...")
    ...
    if cps_with_boundary_df is not None:
        logger.info("6. Change Points with Nearby Boundaries...")
