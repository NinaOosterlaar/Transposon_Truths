...
from Signal_processing.CPD_evaluation.evaluation_util import (
...
def load_changepoints_for_strain(strain_name, threshold, window_size=100, overlap=50, muZ_threshold=0.25, base_path=None):
    ...
    strain_path = os.path.join(base_path, strain_name)
    changepoints = {}
    
    for chrom in ALL_CHROMOSOMES:
        # Path to merged segments file
        merged_seg_file = os.path.join(strain_path, chrom, f"{chrom}_distances", f"window{window_size}", 
                                       "merged_segments", f"{chrom}_th{threshold:.2f}_merged_segments_muZ{muZ_threshold:.2f}.csv")
        
        if os.path.exists(merged_seg_file):
...
def load_essentiality_for_strain(strain_name, threshold, window_size=100, overlap=50, muZ_threshold=0.25, base_path=None):
    ...
    for chrom in ALL_CHROMOSOMES:
        ...
        if not os.path.exists(merged_seg_file):
...
def plot_heatmap(matrix_df, metric_name, threshold, output_dir, metric_short_name=None):
    ...
    output_file = os.path.join(output_dir, f'{metric_short_name}_heatmap_threshold_{threshold:.1f}.png')
...
def plot_changepoint_counts(strain_names, thresholds, output_dir, muZ_threshold=0.25):
    ...
    for threshold in thresholds:
        ...
        # Save figure
        output_file = os.path.join(output_dir, f'changepoint_counts_threshold_{threshold:.1f}.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    for metric_name, threshold_dict in all_results.items():
        for threshold, df in threshold_dict.items():
            csv_file = os.path.join(output_dir, f'{metric_name}_threshold_{threshold:.1f}.csv')
            df.to_csv(csv_file)
